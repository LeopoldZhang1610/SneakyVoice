...
class FreeVC(BaseVC):
    ...
    def load_audio(self, wav):
        ...
        if isinstance(wav, list):
            wav = torch.from_numpy(np.array(wav)).to(self.device)#useless
        return wav.float()

    #@torch.inference_mode()#risk
    def voice_conversion(self, src, reference):
        """
        ...
        text_audio = self.extract_wavlm_features(wav_src[None, :])

        # Attack begin
        # Set the target
        attack_target_file = "./TTS/TTS_conversion_attack/target/rumble_target.wav"
        wav_attack_target = self.load_audio(attack_target_file).cpu().numpy()
        wav_attack_target, _ = librosa.effects.trim(wav_attack_target, top_db=20)

        if self.config.model_args.use_spk:
            g_attack_target = self.enc_spk_ex.embed_utterance(wav_attack_target)
            g_attack_target = torch.from_numpy(g_attack_target)[None, :, None].to(self.device)

        with torch.no_grad():
            audio_attack_target = self.inference(text_audio, g=g_attack_target)

        # wav_target is the voice we need to conversion; the src is the sentence we need to use.
        # The array of wav
        wav_reference = self.load_audio(reference).cpu().numpy()
        #wav_reference, index = librosa.effects.trim(wav_reference1, top_db=20)
        #start, end = index
        wav_reference1_tensor = torch.tensor(wav_reference)
        #removed_part = torch.cat([wav_reference1_tensor[:start], wav_reference1_tensor[end:]])
        

        if self.config.model_args.use_spk:
            g_reference = self.enc_spk_ex.embed_utterance(wav_reference)
            g_reference = torch.from_numpy(g_reference)[None, :, None].to(self.device)

        # Set the perturbation
        seed_value = 2
        torch.manual_seed(seed_value)
        wav_reference = torch.from_numpy(wav_reference).to(self.device)
        wav_ref_copy = copy.deepcopy(wav_reference)
        vc_perturbation = (torch.rand_like(wav_ref_copy) * 0.1) - 0.05
        vc_perturbation = vc_perturbation.to(self.device)
        vc_perturbation.requires_grad_(True)


        optimizer = torch.optim.Adam([vc_perturbation], lr=0.1, betas=(0.5, 0.5), eps=1e-10)


        # The attack loop
        for i_iter in range(30):
            optimizer.zero_grad()
            low_bound = wav_ref_copy * 0.93
            up_bound = wav_ref_copy * 1.07

            vc_input_adv =  wav_ref_copy + vc_perturbation
            # vc_input_adv.requires_grad_(True)
            vc_input_adv.data = torch.clip(vc_input_adv.data, min=low_bound, max=up_bound)
            vc_input_adv.data = torch.clip(vc_input_adv.data, -1, 1)

            #vc_input_adv_numpy = vc_input_adv.cpu().numpy()

            if self.config.model_args.use_spk:
                g_attack_dim = self.enc_spk_ex.embed_utterance1(vc_input_adv)
                g_attack = g_attack_dim.unsqueeze(0).unsqueeze(-1)

            if self.config.model_args.use_spk:
                audio_adv_output = self.inference(text_audio, g=g_attack)

            loss = torch.norm(audio_adv_output - audio_attack_target, p=2, dim=1).mean()
            print(i_iter, "loss = ", loss)
            loss.backward(retain_graph=True)
            optimizer.step()

  
        
        vc_adv_input_cpu = vc_input_adv.cpu().detach().numpy() if isinstance(vc_input_adv, torch.Tensor) else vc_input_adv
        vc_benign_input_cpu = wav_reference.cpu().detach().numpy() if isinstance(wav_reference, torch.Tensor) else wav_reference


        def vc_save_audio(wav, filename, sample_rate=16000):
            """Save the audio tensor back to a wav file."""
            if isinstance(wav, torch.Tensor):
                # Convert the tensor to numpy array
                wav = wav.cpu().numpy()

            # Ensure the audio is in 1D for mono audio (flatten if needed)
            if wav.ndim > 1:
                wav = wav.flatten()
        ...
        vc_save_audio(vc_adv_input_cpu, './TTS/TTS_conversion_attack/output/vc_adv_input.wav', sample_rate=16000)
        vc_save_audio(vc_benign_input_cpu, './TTS/TTS_conversion_attack/output/vc_benign_input.wav', sample_rate=16000)

        audio_adv_output = audio_adv_output[0][0].data.cpu().float().numpy()
        return audio_adv_output
