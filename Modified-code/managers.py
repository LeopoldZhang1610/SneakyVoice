...
class EmbeddingManager(BaseIDManager):
    ...
    def tensor_to_embedding(self, m_input):
        embedding = self.encoder.compute_embedding(m_input)
        return embedding

    def wav_to_wav_2(self, wav_file):
        wave_2 = self.encoder_ap.load_wav_turn(wav_file, sr=self.encoder_ap.sample_rate)# wav file to numpy array
        return wave_2

    def wav_to_tensor(self, wav_file):
        waveform = self.encoder_ap.load_wav(wav_file, sr=self.encoder_ap.sample_rate)# wav file to numpy array
        # waveform = self.encoder_ap.load_wav(wav_file.replace("_1","_1_noise"), sr=self.encoder_ap.sample_rate)
        if not self.encoder_config.model_params.get("use_torch_spec", False):
            ...
            m_input = torch.from_numpy(waveform) # array to tensor

        # if self.use_cuda: # put into gpu
        # m_input = m_input.cuda()
        m_input = m_input.cuda()    # put tensor to gpu
        m_input = m_input.unsqueeze(0) # shape from x_dim to 1*x_dim
        return m_input
    ...
    def compute_embedding_from_clip(self, wav_file: Union[str, List[str]]) -> list:
        ...
        def _compute(wav_file: str):
            ...
            m_input = m_input.cuda()    # put tensor to gpu
            m_input = m_input.unsqueeze(0) # shape from x_dim to 1*x_dim
            embedding = self.encoder.compute_embedding(m_input) # from tensor to embedding
           
            # m_input_adv = copy.deepcopy(m_input)
            # perturbation = torch.zeros_like(m_input)

            seed_value = 1  
            torch.manual_seed(seed_value)
            perturbation = (torch.rand_like(m_input) * 0.1) - 0.05
            perturbation = perturbation.to("cuda")
            perturbation.requires_grad_(True)

            return embedding, m_input, perturbation
        ...
        # return embedding[0].tolist()#return embedding[0], return embedding
        return embedding
