...
def restore_wav(input_tensor, wav_2, filename, sample_rate=16000, r=-27):
    input_array = input_tensor.squeeze().detach().cpu().numpy()

    r = 10 ** (r / 20)
    # restored_input = input_array / np.sqrt((len(input_array) * (r ** 2)) / np.sum(input_array ** 2))
    restored_input = input_array / np.sqrt((len(input_array) * (r ** 2)) / wav_2)

    sf.write(filename, restored_input, sample_rate)
    ...
    def tts(
        ...
        **kwargs,
        ...
        if (
            ...
            speaker_embedding, input, perturbation = self.tts_model.speaker_manager.compute_embedding_from_clip(speaker_wav)
       
        optimizer = torch.optim.Adam([perturbation], lr=0.1, betas=(0.5,0.5), eps=1e-10)

        vocoder_device = "cpu" # put into gpu
        # vocoder_device = "gpu"
            ...
            for sen in sens:
                if hasattr(self.tts_model, "synthesize"):
                    ...
                    outputs = synthesis(
                        ...
                        d_vector=speaker_embedding, # benign voice reference
                        language_id=language_id,
                    )
                    
                   
                    target_file = "/home/zhc/TTS/target/bi_tone_two_sec.wav"
                    target_tensor = self.tts_model.speaker_manager.wav_to_tensor(target_file)
                    target_embedding = self.tts_model.speaker_manager.tensor_to_embedding(target_tensor)
                    target = synthesis(
                        model=self.tts_model,
                        text=sen,
                        CONFIG=self.tts_config,
                        ...
                        style_text=style_text,
                        use_griffin_lim=use_gl,
                        d_vector=target_embedding, # target voice reference
                        language_id=language_id,
                    )                   
                    for i_iter in range(50):
                        optimizer.zero_grad()
                        lower_bound = input * 0.93
                        upper_bound = input * 1.07
                        # perturbation.data = torch.clip(perturbation.data, -0.1, 0.1)
                        input_adv = perturbation + input
                        input_adv.data = torch.clip(input_adv.data, min=lower_bound, max=upper_bound)
                        input_adv.data = torch.clip(input_adv.data, -1, 1)
                       
                        speaker_embedding_adv = self.tts_model.speaker_manager.tensor_to_embedding(input_adv)
                        outputs_adv = synthesis(
                            model=self.tts_model,
                            text=sen,
                            CONFIG=self.tts_config,
                            use_cuda=self.use_cuda,
                            ...
                            style_text=style_text,
                            use_griffin_lim=use_gl,
                            d_vector=speaker_embedding_adv, # adversarial voice reference
                            language_id=language_id,
                        )

                        min_length = min(outputs_adv["outputs"]["model_outputs"][0][0].shape[0], target["outputs"]["model_outputs"][0][0].shape[0])
                        a_trimmed = outputs_adv["outputs"]["model_outputs"][0,:,:min_length]
                        bi_trimmed = target["outputs"]["model_outputs"][0,:,:min_length]
                        loss =  torch.norm(a_trimmed - bi_trimmed, p=2, dim=1).mean() # computing loss
                        # loss =  torch.norm(embedding_adv - embedding_beep, p=2, dim=1).mean()
                    ...
                    wav_2 = self.tts_model.speaker_manager.wav_to_wav_2(speaker_wav)

                    input_adv_save_dir = "./TTS/output/Adv_input_target_beep_test1.wav"
                    restore_wav(input_adv, wav_2, input_adv_save_dir, sample_rate=16000)
                   
                    


                waveform = outputs["wav"]
                waveform_adv = outputs_adv["wav"]
                if not use_gl:#skip
                ...
                if "do_trim_silence" in self.tts_config.audio and self.tts_config.audio["do_trim_silence"]:
                    waveform = trim_silence(waveform, self.tts_model.ap)#skip

                wavs += list(waveform)
                wavs += [0] * 10000
                wavs_adv += list(waveform_adv)
                wavs_adv += [0] * 10000
        else:
